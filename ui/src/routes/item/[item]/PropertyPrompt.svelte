...
{#if loggedIn}
	{#if request}
		{#await request}
			<div class="field-group grid-cols-[auto_1fr_auto]">
				<div class="btn preset-tonal">
					<Icon data={faPlus} class="fa-fw"></Icon>
				</div>
				<div class="input preset-tonal text-center">
		...
		{:then value}
			...
			{#if !value.ok}
				<div class="field-group grid-cols-[auto_1fr_auto]">
					<button class="btn preset-tonal-warning" onclick={reset}>
						<Icon data={faXmark} class="fa-fw"></Icon>
					</button>
					<div class="input preset-tonal-warning text-center">
						{value.statusText}
			...
			{:else if true}
				<!--Accepted-->
				<div class="field-group grid-cols-[auto_1fr_auto]">
					<button class="btn preset-tonal-success" onclick={reset}>
						<Icon data={faCheck} class="fa-fw"></Icon>
					</button>
					<div class="input preset-tonal-success text-center">Submitted: Pending Approval</div>
					<button class="btn preset-tonal-success" onclick={reset}>
			...
			{:else}
				...
				<div class="field-group grid-cols-[auto_1fr_auto]">
					...
					<div class="input preset-tonal-warning text-center">Rejected: {'reason'}</div>
					<button class="btn preset-tonal-warning" onclick={reset}>
						<Icon data={faArrowRotateLeft} class="fa-fw"></Icon>
					</button>
				</div>
	...
	{:else}
		<form
			class="field-group grid-cols-[auto_auto_1fr_auto_auto]"
			onsubmit={(e) => {
				e.preventDefault();
				submitNew();
			}}
		>
			...
			<select class="select capitalize" bind:value={propClass}>
